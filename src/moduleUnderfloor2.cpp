...
moduleUnderfloor2 wpUnderfloor2;

moduleUnderfloor2::moduleUnderfloor2() {
	// section to config and copy
	ModuleName = "Underfloor2";
	mb = new moduleBase(ModuleName);
	Pin = D2;
}
void moduleUnderfloor2::init() {
	pinMode(Pin, OUTPUT);
	output = false;
	autoValue = false;
	handValue = false;
	...
	// section to copy
	mb->initDebug(wpEEPROM.addrBitsDebugModules2, wpEEPROM.bitsDebugModules2, wpEEPROM.bitDebugUnderfloor2);
	mb->initCalcCycle(wpEEPROM.byteCalcCycleUnderfloor2);
}

//###################################################################################
// public
//###################################################################################
void moduleUnderfloor2::cycle() {
...
void moduleUnderfloor2::publishSettings() {
	publishSettings(false);
}
void moduleUnderfloor2::publishSettings(bool force) {
	if(force) {
...
void moduleUnderfloor2::publishValues() {
	publishValues(false);
}
void moduleUnderfloor2::publishValues(bool force) {
	if(force) {
		publishOutputLast = 0;
		publishAutoValueLast = 0;
		publishHandValueLast = 0;
...
void moduleUnderfloor2::setSubscribes() {
...
void moduleUnderfloor2::checkSubscribes(char* topic, String msg) {
	if(strcmp(topic, mqttTopicSetHand.c_str()) == 0) {
		bool readSetHand = msg.toInt();
		if(handSet != readSetHand) {
			handSet = readSetHand;
			bitWrite(wpEEPROM.bitsSettingsModules2, wpEEPROM.bitUnderfloor2Hand, handSet);
...
void moduleUnderfloor2::SetHandValueSet(bool val) {
	handValueSet = val;
	bitWrite(wpEEPROM.bitsSettingsModules2, wpEEPROM.bitUnderfloor2HandValue, handValueSet);
	EEPROM.write(wpEEPROM.addrBitsSettingsModules2, wpEEPROM.bitsSettingsModules2);
	EEPROM.commit();
	wpFZ.DebugWS(wpFZ.strINFO, "SetHandValueSet", "save to EEPROM: 'module" + ModuleName + "::handValueSet' = " + String(handValueSet));
}
void moduleUnderfloor2::InitSetPoint(uint8 setpoint) {
	setPoint = setpoint;
}
void moduleUnderfloor2::SetSetPoint(uint8 setpoint) {
	setPoint = setpoint;
	EEPROM.write(wpEEPROM.byteUnderfloor2Setpoint, setPoint);
	EEPROM.commit();
	wpFZ.DebugWS(wpFZ.strINFO, "SetSetPoint", "save to EEPROM: 'module" + ModuleName + "::SetSetPoint' = " + String(setPoint));
}
String moduleUnderfloor2::SetTopicTempUrl(String topic) {
	mqttTopicTemp = topic;
...
// private
//###################################################################################
void moduleUnderfloor2::publishValue() {
...
void moduleUnderfloor2::calcOutput() {
	if(setPoint < temp) {
		autoValue = false;
	}
	if(setPoint > temp) {
...
// section to copy
//###################################################################################
uint16 moduleUnderfloor2::getVersion() {
	String SVN = "$Rev: 198 $";
...
void moduleUnderfloor2::changeDebug() {
	mb->changeDebug();
}
bool moduleUnderfloor2::Debug() {
	return mb->debug;
}
bool moduleUnderfloor2::Debug(bool debug) {
	mb->debug = debug;
	return true;
}
uint32 moduleUnderfloor2::CalcCycle(){
	return mb->calcCycle;
}
uint32 moduleUnderfloor2::CalcCycle(uint32 calcCycle){
