...
moduleUnderfloor1 wpUnderfloor1;

moduleUnderfloor1::moduleUnderfloor1() {
	// section to config and copy
	ModuleName = "Underfloor1";
	mb = new moduleBase(ModuleName);
	Pin = D1;
}
void moduleUnderfloor1::init() {
	pinMode(Pin, OUTPUT);
	output = false;
	autoValue = false;
	handValue = false;
	...
	// section to copy
	mb->initDebug(wpEEPROM.addrBitsDebugModules2, wpEEPROM.bitsDebugModules2, wpEEPROM.bitDebugUnderfloor1);
	mb->initCalcCycle(wpEEPROM.byteCalcCycleUnderfloor1);
}

//###################################################################################
// public
//###################################################################################
void moduleUnderfloor1::cycle() {
...
void moduleUnderfloor1::publishSettings() {
	publishSettings(false);
}
void moduleUnderfloor1::publishSettings(bool force) {
	if(force) {
...
void moduleUnderfloor1::publishValues() {
	publishValues(false);
}
void moduleUnderfloor1::publishValues(bool force) {
	if(force) {
		publishOutputLast = 0;
		publishAutoValueLast = 0;
		publishHandValueLast = 0;
...
void moduleUnderfloor1::setSubscribes() {
...
void moduleUnderfloor1::checkSubscribes(char* topic, String msg) {
	if(strcmp(topic, mqttTopicSetHand.c_str()) == 0) {
		bool readSetHand = msg.toInt();
		if(handSet != readSetHand) {
			handSet = readSetHand;
			bitWrite(wpEEPROM.bitsSettingsModules2, wpEEPROM.bitUnderfloor1Hand, handSet);
...
void moduleUnderfloor1::SetHandValueSet(bool val) {
	handValueSet = val;
	bitWrite(wpEEPROM.bitsSettingsModules2, wpEEPROM.bitUnderfloor1HandValue, handValueSet);
	EEPROM.write(wpEEPROM.addrBitsSettingsModules2, wpEEPROM.bitsSettingsModules2);
	EEPROM.commit();
	wpFZ.DebugWS(wpFZ.strINFO, "SetHandValueSet", "save to EEPROM: 'module" + ModuleName + "::handValueSet' = " + String(handValueSet));
}
void moduleUnderfloor1::InitSetPoint(uint8 setpoint) {
	setPoint = setpoint;
}
void moduleUnderfloor1::SetSetPoint(uint8 setpoint) {
	setPoint = setpoint;
	EEPROM.write(wpEEPROM.byteUnderfloor1Setpoint, setPoint);
	EEPROM.commit();
	wpFZ.DebugWS(wpFZ.strINFO, "SetSetPoint", "save to EEPROM: 'module" + ModuleName + "::SetSetPoint' = " + String(setPoint));
}
String moduleUnderfloor1::SetTopicTempUrl(String topic) {
	mqttTopicTemp = topic;
...
// private
//###################################################################################
void moduleUnderfloor1::publishValue() {
...
void moduleUnderfloor1::calcOutput() {
	if(setPoint < temp) {
		autoValue = false;
	}
	if(setPoint > temp) {
...
// section to copy
//###################################################################################
uint16 moduleUnderfloor1::getVersion() {
	String SVN = "$Rev: 198 $";
...
void moduleUnderfloor1::changeDebug() {
	mb->changeDebug();
}
bool moduleUnderfloor1::Debug() {
	return mb->debug;
}
bool moduleUnderfloor1::Debug(bool debug) {
	mb->debug = debug;
	return true;
}
uint32 moduleUnderfloor1::CalcCycle(){
	return mb->calcCycle;
}
uint32 moduleUnderfloor1::CalcCycle(uint32 calcCycle){
